...
/**
 * Simulates all-in spot executions for backtests without calling an exchange.
 */
public class SimulatedExecutionService implements ExecutionService {
    ...
    private Result handleBuy(
        ...
        if (snapshot.availableCash().signum() <= 0) {
            return new Result(Status.SKIPPED, false, false, "BUY ignored because no cash is available.");
        }
